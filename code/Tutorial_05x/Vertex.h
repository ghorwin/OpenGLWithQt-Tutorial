...
/*! Storage member that encapsulates vertex data, i.e. position (x,y,z coords) and
	color (r,g,b tuple).

	The data is stored in two QVector3D, which end up in a sequentiel memory occupying
	2x3 floats.

	Note: offsetof(Vertex, m_position) yields the number of bytes between pointer to this
		  Vertex object and the m_position member.

	For 64bit floats (=4 Bytes), we have:
	  offset(m_position) = 0
	  offset(m_color) = 3*4 = 12

	Stride is the size of the Vertex object = 24 bytes (= 2*3*4 Bytes).
*/
class Vertex {
public:
	// Constructors
