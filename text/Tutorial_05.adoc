...
.Nummerierung der Knoten (Vertices) der Box (oben), flächenbezogene Nummerierung (unten)
...
Nun sind die Boxen also bereits im Weltenkoordinatensystem verankert und der Vertexpuffer und Indexpuffer können befüllt werden. Das passiert in der Funktion `copy2Buffer()`.

Die folgende Abbildung zeigt das Ziel dieser Kopieraktion, wobei `copy2Buffer()` jeweils alle Daten für eine Box kopiert.

.Speicherlayout des Vertexpuffers
image::Tutorial_05_vertexbuffer.png[Box Vertex Numbering, pdfwidth=8cm]

Jede Seite der Box wird nacheinander im VBO abgelegt, wobei je Seite 4 Vertexes mit je Koordinaten und Farbwerten abgelegt werden. In der Funktion `copy2Buffer()` wird daher zunächst ein Vektor mit Farben für jeden _seitenbezogenen Vertex_ angelegt, d.h. bei 6 Seiten und 4 Vertices je Seite = 24 Farbwerte.
