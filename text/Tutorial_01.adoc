...
Bleibt noch das Aufräumen der reservierten Resourcen. Hierbei gibt es zu beachten, dass der QOpenGLContext, welche in der Initialisierung für diese Anwendung neu erstellt wurde, nur während der Event-Loop auf der "Surface" arbeiten kann (die Details dazu sind leider nicht genau dokumentiert, aber wenn man nach Ende der Event-Loop im Destruktor der Klasse 
...
aufruft, schlägt dieser Aufruf fehl. Alle weiteren Funktionsaufrufe, welche einen gültigen OpenGLContext verlangen, schlagen daraufhin fehl (manche geben eine `qWarning()` aus). Daher muss man die OpenGL-Resourcen beim Schließen des Fensters aber noch in der Ereignisschleife aufräumen. Daher überlädt man am Besten die Funktion `event()`:
